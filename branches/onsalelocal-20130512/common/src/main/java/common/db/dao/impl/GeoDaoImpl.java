...
@Repository
public class GeoDaoImpl extends GenericDaoImpl implements common.db.dao.GeoDao {
    ...
    public List<City> nearbyCities(Float lat, Float lng, float radius) {
        ...
        String sql = "select ((ACOS(SIN(:lat * PI() / 180) * SIN(lat * PI() / 180) + COS(:lat * PI() / 180) * COS(lat * PI() / 180) * COS((:lng - lon) * PI() / 180)) * 180 / PI()) * 60 * 1.1515) AS `distance`" +
        		", city, state, lat, lon, id from geo.uscities" +
        ...
        List<City> list = this.getNamedParameterJdbcTemplate().query(sql, params, new RowMapper<City>() {
            ...
            public City mapRow(ResultSet rs, int rowNum) throws SQLException {
                ...
                city.setLat(rs.getFloat(4));
                city.setLon(rs.getFloat(5));
                city.setId(rs.getString(6));
                return city;
            }
        });
        return list;
	...
	public City getCity(String cityId) {
        String sql = "select * from geo.uscities where id=:id";
        return queryUniqueBySql(City.class, Collections.singletonMap("id", cityId), sql);
	}
}
