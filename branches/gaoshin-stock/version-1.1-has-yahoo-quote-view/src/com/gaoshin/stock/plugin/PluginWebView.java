    ...
    public class GenericWebViewClient extends WebViewClient {
        ...
        public GenericWebViewClient() {
            postRunnable = new Runnable() {
                ...
                public void run() {
                    if(newPage) {
                        loadUrl("javascript:try{if((typeof android != 'undefined') && 'loaded|complete'.indexOf(document.readyState) != -1){android.onDomReady();}}catch(e){alert(e);}");
...
//            }
    ...
    private void postAction() {
        ...
        try {
            String script = "";
            if(plugin.getType() != PluginType.System.ordinal() && paramList.getItems().size()>0) {
                script = "try {" +
