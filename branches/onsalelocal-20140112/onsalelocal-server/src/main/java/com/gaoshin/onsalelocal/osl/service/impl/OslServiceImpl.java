...
import com.gaoshin.onsalelocal.api.OfferStatus;
...
import com.gaoshin.onsalelocal.osl.entity.NotificationDetail;
...
//            	user.setMyFollower(followed);
		...
		int updated = favouriteDao.update("update Offer set views = views+1, popularity=popularity+1 where id=?", offerId);
    ...
    public void submitOffer(Offer offer) {
		...
		assertNotNull(offer.getAddress(), "address");
		assertNotNull(offer.getCity(), "city");
		assertNotNull(offer.getState(), "state");
		assertNotNull(offer.getCountry(), "country");
		assertNotNull(offer.getLatitude(), "latitude");
		assertNotNull(offer.getLongitude(), "longitude");
		assertNotNull(offer.getMerchant(), "merchant");
    ...
    public Store createStore(String userId, Store store) {
		...
		if(store.getLogo() == null) {
			store.setLogo("http://maps.googleapis.com/maps/api/staticmap?center=" + store.getLatitude() + "," + store.getLongitude() + "&zoom=16&size=600x300&sensor=false");
		}
    ...
    public List<Company> listCompanies(int offset, int size) {
	    return oslDao.query(Company.class, null);
    }
}
