jQuery(document).ready(function($) {
	...
	$(document).on('click', '.ipin-like', function() {
		var like = $(this);
		var	offerid = like.data('post_id');
	    var url = serverBase + "/ws/v2/offer/like/" + offerid;
	    $.ajax({
	          url:url,
	          type:"POST",
	          ...
	          complete: function(transport) {
	                 if(transport.status == 200) {
	                	 var cnt = parseInt($("#likes-count-" + offerid).html().split(" ")[0]);
	                	 cnt = cnt + 1;
	                	 $("#likes-count-" + offerid).html(cnt + " likes");
	                	 $(like).parent().hide();
	                 } else {
