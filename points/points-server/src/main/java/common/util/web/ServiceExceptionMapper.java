		...
		try {
            ...
            if (html) {
		        ...
		        try {
		            ...
		            String oopath = uri + "/" + BusinessException.class.getSimpleName() + "/" + exception.getServiceError().name() + "/index.jsp.oo";
				...
				return Response.status(exception.getServiceError().getCode()).type(MediaType.APPLICATION_JSON_TYPE).build();
			}
			else {
				String xmlStr = getXml(data);
                responseInvoker.get().getWriter().write(xmlStr);
				return Response.status(exception.getServiceError().getCode()).type(MediaType.TEXT_XML_TYPE).build();
			}
		} catch (Exception e) {
