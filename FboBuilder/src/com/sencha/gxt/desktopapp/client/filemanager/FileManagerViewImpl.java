...
public class FileManagerViewImpl implements FileManagerView, HideHandler {
	...
	private static final String TITLE = "Assets Manager";
	...
	public FileManagerViewImpl(FileManagerPresenter fileManagerPresenter,
	...
	public void collapse() {
		FileModel fileModel = getTreeGrid().getSelectionModel()
				.getSelectedItem();
		if (fileModel == null) {
	...
	/**
	 * Works around a minor issue with TreeGrid and program initiated
	 * GridEditing in which the selection is lost if the user clicks in the
	 * active EditField.
	 */
	private void editRestoreFileModel() {
		if (editFileModel != null) {
			getTreeGrid().getSelectionModel().select(editFileModel, true);
		}
	}
				...
				public void onCompleteEdit(CompleteEditEvent<FileModel> event) {
					...
					Scheduler.get().scheduleFixedDelay(new RepeatingCommand() {
						...
						public boolean execute() {
							getWindow().setHeadingText(
									getTitle(getSelectedItem()));
							return false;
						}
					}, 250);
				}
	...
	private ColumnConfig<FileModel, Date> getDateConfig() {
		if (dateConfig == null) {
			dateConfig = new ColumnConfig<FileModel, Date>(
					getFileModelProperties().lastModified(), 100, "Date");
			dateConfig.setCell(new DateCell(DateTimeFormat
	...
	private FileManagerToolBar getFileManagerToolBar() {
		if (fileManagerToolBar == null) {
			fileManagerToolBar = new FileManagerToolBar(
					getFileManagerPresenter());
	...
	private ColumnConfig<FileModel, String> getNameConfig() {
		if (nameConfig == null) {
			nameConfig = new ColumnConfig<FileModel, String>(
		...
		if (selectionChangedHandler == null) {
			selectionChangedHandler = new SelectionChangedHandler<FileModel>() {
				...
				public void onSelectionChanged(
						SelectionChangedEvent<FileModel> event) {
					FileModel fileModel = treeGrid.getSelectionModel()
							.getSelectedItem();
					if (fileModel != null) {
	...
	private ColumnConfig<FileModel, Long> getSizeConfig() {
		if (sizeConfig == null) {
			sizeConfig = new ColumnConfig<FileModel, Long>(
					getFileModelProperties().size(), 100, "Size");
	...
	private String getTitle(FileModel fileModel) {
		return fileModel == null ? TITLE : TITLE + " - "
				+ getFileSystem().getPath(fileModel);
	}

	private FileManagerTreeGrid getTreeGrid() {
		if (treeGrid == null) {
			treeGrid = new FileManagerTreeGrid(getFileSystem().getTreeStore(),
					getColumnModel(), getNameConfig());
			treeGrid.getView()
					.setEmptyText(
							"Use tool bar or context menu to create files and folders.");
			...
			treeGrid.getSelectionModel().addSelectionChangedHandler(
	...
	private TreeGridDragSource<FileModel> getTreeGridDragSource() {
		if (treeGridDragSource == null) {
			treeGridDragSource = new TreeGridDragSource<FileModel>(
					getTreeGrid());
		}
	...
	private TreeGridDropTarget<FileModel> getTreeGridDropTarget() {
		if (treeGridDropTarget == null) {
			treeGridDropTarget = new TreeGridDropTarget<FileModel>(
					getTreeGrid());
	...
	private ColumnConfig<FileModel, ?> getTypeConfig() {
		if (typeConfig == null) {
			typeConfig = new ColumnConfig<FileModel, FileType>(
					getFileModelProperties().fileType(), 100, "Type");
		}
		return typeConfig;
	}
	...
	private VerticalLayoutContainer getVerticalLayoutContainer() {
		if (verticalLayoutContainer == null) {
			...
			verticalLayoutContainer.add(getFileManagerToolBar(),
					new VerticalLayoutData(1, -1));
			verticalLayoutContainer.add(getTreeGrid(), new VerticalLayoutData(
					1, 1));
		}
