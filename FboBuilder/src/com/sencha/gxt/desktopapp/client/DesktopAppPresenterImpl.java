...
import com.sencha.gxt.desktopapp.client.canvas.CanvasPresenterImpl;
...
import com.sencha.gxt.desktopapp.client.filemanager.FileManagerPresenter;
...
import com.sencha.gxt.desktopapp.client.property.PropertyPresenter;
import com.sencha.gxt.desktopapp.client.property.PropertyPresenterImpl;
...
public class DesktopAppPresenterImpl implements DesktopAppPresenter {
	...
	private static final String STORAGE_KEY_PREFIX_SYSTEM = STORAGE_KEY_PREFIX
			+ ".system";
	private static final String STORAGE_KEY_PREFIX_USER = STORAGE_KEY_PREFIX
			+ ".user";
	...
	private Map<String, BrowserPresenterImpl> browserMap = new HashMap<String, BrowserPresenterImpl>();
	private BrowserPresenterImpl browserPresenter = null;
	private PropertyHandler propertyHandler;

	/**
	 * Creates a desktop application presenter that communicates with the rest
	 * of the application using the specified desktop bus.
	 * 
	 * @param desktopBus
	 *            the application bus
	 */
	public DesktopAppPresenterImpl(DesktopBus desktopBus) {
		...
		desktopBus.addPropertyHandler(getPropertyHandler());
	}
	...
	public FileSystem getFileSystem() {
		if (fileSystem == null) {
			...
			fileSystem.getTreeStore().addStoreUpdateHandler(
					getStoreUpdateHandler());
			fileSystem.getTreeStore().addStoreRemoveHandler(
					getStoreRemoveHandler());
		}
		return fileSystem;
	}
	...
	public void go(HasWidgets hasWidgets) {
		...
		getDesktopAppView().setDesktopLayoutType(
				getProfile().getDesktopLayoutType());
	}
	...
	public void onOpenFileManager() {
		if(fileManagerPresenter == null) {
			fileManagerPresenter = new FileManagerPresenterImpl(getFileSystem(), getDesktopBus());
			fileManagerPresenter.go(getDesktopAppView());
		}
		else {
			fileManagerPresenter.go(getDesktopAppView());
		}
	...
	@Override
	public DesktopAppPresenter.ProfileStatus onUpdateProfile(
			ProfileModel profileModel) {
		boolean isUpdate = profileModel != null;
		if (isUpdate) {
			setProfile(profileModel);
			getDesktopAppView().setDesktopLayoutType(
					profileModel.getDesktopLayoutType());
		}
		getDesktopBus().fireUpdateProfileEvent(
				new UpdateProfileEvent(profileModel, isUpdate));
	...
	private BackingStore createBackingStore(String storageKeyPrefix) {
		...
		if (!isLocalStorageSupportChecked()) {
			...
			throw new IllegalStateException(
					"Must invoke isLocalStorageSupported before invoking getBackingStore");
		}
		if (!isLocalStorageSupported()) {
			backingStore = new MemoryBackingStore(storageKeyPrefix);
		} else {
			backingStore = new StorageBackingStore(getStorageProvider()
					.getStorage(), storageKeyPrefix);
		}
		return backingStore;
	}
	...
	private void createNewUser(String userName) {
		...
		AutoBean<ProfileModel> profileAutoBean = getProfileAutoBeanFactory()
				.profileModel();
	...
	private OpenFileModelHandler getOpenFileModelHandler() {
		if (openFileModelHandler == null) {
			openFileModelHandler = new OpenFileModelHandler() {
				@Override
				public void onOpenFileModel(
						OpenFileModelEvent openFileModelEvent) {
	...
	protected void showProperty(PropertyEvent propertyEvent) {
		PropertyPresenter presenter = null;
			if(presenter == null) {
				presenter = new PropertyPresenterImpl(getDesktopBus());
			}
			presenter.showProperty(propertyEvent.toString());
		if (presenter != null) {
	...
	private ProfileModel getProfile(String userName) {
		...
		ProfileModel profileModel = AutoBeanCodex.decode(
				getProfileAutoBeanFactory(), ProfileModel.class, payload).as();
		return profileModel;
	}
			...
			storeAddHandler = new StoreAddHandler<FileModel>() {
				...
				public void onAdd(StoreAddEvent<FileModel> event) {
					for (FileModel fileModel : event.getItems()) {
						getDesktopBus().fireAddFileModelEvent(
								new AddFileModelEvent(fileModel));
					}
				}
			};
		}
		...
		if (storeRemoveHandler == null) {
			storeRemoveHandler = new StoreRemoveHandler<FileModel>() {
				...
				public void onRemove(StoreRemoveEvent<FileModel> event) {
					getDesktopBus().fireRemoveFileModelEvent(
							new RemoveFileModelEvent(event.getItem()));
					List<FileModel> children = ((TreeStoreRemoveEvent<FileModel>) event)
							.getChildren();
					for (FileModel child : children) {
						getDesktopBus().fireRemoveFileModelEvent(
								new RemoveFileModelEvent(child));
					}
				}
			};
		}
		...
		if (storeUpdateHandler == null) {
			storeUpdateHandler = new StoreUpdateHandler<FileModel>() {
				...
				public void onUpdate(StoreUpdateEvent<FileModel> event) {
					for (FileModel fileModel : event.getItems()) {
						getDesktopBus().fireUpdateFileModelEvent(
								new UpdateFileModelEvent(fileModel));
					}
				}
			};
		}
	...
	private void openFileModel(OpenFileModelEvent openFileModelEvent) {
		...
		case BOOKMARK:
			if(browserPresenter == null) {
				browserPresenter = new BrowserPresenterImpl(getDesktopBus(), fileSystem,
					fileModel);
			}
			else {
				browserPresenter.getFileBasedMiniAppView().setValue(fileModel.getName());
			}
			presenter = browserPresenter;
			break;
		case DOCUMENT:
			presenter = new WordProcessorPresenterImpl(getDesktopBus(),
		...
		case FOLDER:
			...
			break;
		case PROGRAM:
			presenter = new InterpreterPresenterImpl(getDesktopBus(),
					fileSystem, fileModel);
			break;
		case SPREADSHEET:
			presenter = new SpreadsheetPresenterImpl(getDesktopBus(),
					fileSystem, fileModel);
			break;
		default:
			Info.display("Desktop", "You opened " + fileModel.getName()
					+ ", which is of type " + fileModel.getFileType()
	...
	private void setProfile(ProfileModel profileModel, String userName) {
		String payload = AutoBeanCodex.encode(
				AutoBeanUtils.getAutoBean(profileModel)).getPayload();
		getUserBackingStore(userName).setItem(PROFILE_KEY, payload);
	}

	@Override
	public void onCanvas() {
	    new CanvasPresenterImpl(getDesktopBus()).go(getDesktopAppView());
	}

	@Override
    public void onProperty() {
	    new PropertyPresenterImpl(getDesktopBus()).go(getDesktopAppView());
    }
