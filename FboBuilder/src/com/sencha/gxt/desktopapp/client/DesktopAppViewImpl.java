...
public class DesktopAppViewImpl implements DesktopAppView {
  ...
  private Shortcut canvasShortcut;
  ...
  private SelectHandler canvasShortcutListener;
  private DesktopAppPresenter desktopAppPresenter;
private Shortcut propertyShortcut;
private SelectHandler propertyShortcutListener;
  ...
  private Desktop getDesktop() {
    if (desktop == null) {
      ...
      desktop.addShortcut(getCanvasShortcut());
      desktop.addShortcut(getPropertyShortcut());
  ...
  private Shortcut getCanvasShortcut() {
	    if (canvasShortcut == null) {
	      canvasShortcut = new Shortcut();
	      canvasShortcut.setText("Canvas");
	      canvasShortcut.setIcon(DesktopImages.INSTANCE.palette48());
	      canvasShortcut.addSelectHandler(getCanvasShortcutListener());
	    }
	    return canvasShortcut;
	  }

  private Shortcut getPropertyShortcut() {
	    if (propertyShortcut == null) {
	      propertyShortcut = new Shortcut();
	      propertyShortcut.setText("Properties");
	      propertyShortcut.setIcon(DesktopImages.INSTANCE.info48());
	      propertyShortcut.addSelectHandler(getPropertyShortcutListener());
	    }
  ...
  private SelectHandler getCanvasShortcutListener() {
	    if (canvasShortcutListener == null) {
	      canvasShortcutListener = new SelectHandler() {
	        @Override
	        public void onSelect(SelectEvent event) {
	          getDesktopAppPresenter().onCanvas();
	        }
	      };
	    }
	    return canvasShortcutListener;
	  }

  private SelectHandler getPropertyShortcutListener() {
	    if (propertyShortcutListener == null) {
	      propertyShortcutListener = new SelectHandler() {
	        @Override
	        public void onSelect(SelectEvent event) {
	          getDesktopAppPresenter().onProperty();
	        }
	      };
	    }
	    return propertyShortcutListener;
	  }
