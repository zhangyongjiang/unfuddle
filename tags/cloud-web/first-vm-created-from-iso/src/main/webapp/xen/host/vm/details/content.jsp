...
<div id="tabs" style="min-height: 480px;">
	...
	<div id="summary">
		<div>
			...
			<c:if test="${!it.template}">
				...
				<c:if test="${!it.template && it.powerState == 'RUNNING'}">
				...
				<c:if test="${!it.template && it.powerState == 'HALTED'}">
					<button id="btnStart" onClick='startVm("${it.hostId}", "${it.uuid}")'>Start</button>
				</c:if>
				<c:if test="${!it.template && it.powerState == 'SUSPENDED'}">
		...
		<td>
			<c:forEach var="console" items="${it.consoleList }">
				<BUTTON onClick='openConsole("${console.hostId}", "${console.vmId}", "${console.uuid}")'>Open</BUTTON>
			</c:forEach>
		</td></tr>
		</c:if>
	</table>
	...
	<c:if test="${not empty it.vifList }">
		...
		<c:forEach var="vif" items="${it.vifList }">
			<li>${vif.uuid } ${vif.device } ${vif.statusDetail }</li>
		</c:forEach>
		</ul>
	</c:if>
	</div>
...
function openConsole(hostId, vmId, consoleId) {
	...
	window.open(url,'name','height=520,width=800, toolbar=no,directories=no,status=no,menubar=no, scrollbars=no,resizable=no'); 
}
</script>