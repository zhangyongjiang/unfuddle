...
public class JsonUtil {
    ...
    public static String toJsonStr(Element ele) throws Exception {
        return toJson(ele).toString();
    }

    public static JSONObject toJson(Element ele) throws Exception {
        if (ele == null)
            return null;
        ...
        for (int i = 0; i < nlist.getLength(); i++) {
            ...
            if (node.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (isTextElement((Element) node)) {
                    add(jobj, nodeName, node.getFirstChild().getNodeValue());
                } else {
                    add(jobj, nodeName, toJson((Element) node));
                }
            } else if (node.getNodeType() == Node.ATTRIBUTE_NODE) {
                jobj.put("@" + node.getNodeName(), node.getNodeValue());
            }
        }
        return jobj;
    }

    private static boolean isTextElement(Element elem) {
