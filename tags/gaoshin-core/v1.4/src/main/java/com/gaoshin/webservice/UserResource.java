...
public class UserResource extends JerseyBaseResource {
    ...
    @Path("icon/{userId}")
    @Produces("image/*")
    public Response downloadPhotoByUserId(@PathParam("userId") Long userId) throws IOException {
        User user = new User();
        user.setId(userId);
    ...
    public UserAndDistanceList search(
            ...
            @DefaultValue("3000") @QueryParam("miles") Float miles, @QueryParam("dims") String dims,
        ...
        UserAndDistanceList list = userService.search((selfExcluded ? user : null), latitude, longitude, latitude1, longitude1, miles, minAge, maxAge, dims, offset, size);
        return list;
    }
    ...
    public User getCurrentUser() {
        ...
        boolean found = false;
System.out.println(SessionKeyUser + ":" + SessionKeyUser);
        if (user == null) {
            user = new User();

            Long userId = (Long) getSessionAttribute(SessionKeyUserId);
System.out.println(SessionKeyUserId + ":" + userId);
            if (userId != null) {
                user.setId(userId);
                found = true;
            }
...
System.out.println("Username in cookie is " + userName);
            if (userName != null) {
...
System.out.println("phone in cookie is " + userName);
            if (phone != null) {
                user.setPhone(phone);
                found = true;
            }
