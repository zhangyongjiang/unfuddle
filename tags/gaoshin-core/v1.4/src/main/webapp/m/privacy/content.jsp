...
<jsp:include page="/privacy/list?format=object&var=privacys"></jsp:include>
<c:forEach var="privacy" items="${privacys.list}">
	<h4>
	<c:if test="${me.role =='ADMIN' || me.role =='SUPER'}">
	<a href='#' onclick='deletePrivacy(${privacy.id})'>
	</c:if>
	${privacy.title}
	<c:if test="${me.role =='ADMIN' || me.role =='SUPER'}">
	</a>
...
<c:if test="${me.role =='ADMIN' || me.role =='SUPER'}">
	...
	<strong>Write One</strong>
		<form onsubmit="return createPrivacy();">
...
function createPrivacy() {
    ...
    if(content == null || content.trim().length == 0) {
        alert("Privacy content is required");
        return false;
    }
    var req = {
        title: title,
    ...
    var url = '<c:url value="/privacy/submit"/>';
	$.ajax({
		url : url,
		type : "POST",
        data:json,
