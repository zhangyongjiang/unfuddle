<%@page import="com.gaoshin.beans.DimensionValue"%>
...
while(cat.getParent() != null && !cat.getName().equals("Shopping")){
    ...
    cat = cat.getParent();
%>
<div style="float:left;width:120px;padding:8px;"><a href="/shopping/create/need/<%=parent.getId() %>"><%= parent.getName() %></a></div>
<%
}
%>
...
<c:forEach var="subcat" items="${it.children}">
	<div style="width:120px;float:left;padding:8px;"><a href="/shopping/create/need/${subcat.id}">${subcat.name }</a></div>
</c:forEach>
</c:if>
...
<%=dim.getName() %><br/>
<%
		for(DimensionValue dv : dim.getValues()) {
%>
<li><%= dv.getDvalue() %></li>
<%		    
		}
