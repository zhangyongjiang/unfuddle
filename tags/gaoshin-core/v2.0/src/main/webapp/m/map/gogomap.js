...
POIMap.prototype.initView = function (domId, defLat, defLng) {
    ...
    var center = new GLatLng(defLat, defLng);
...
//    this.map.addControl(new GMapTypeControl(), new GControlPosition(G_ANCHOR_TOP_RIGHT, new GSize(8, 8)));
    ...
    GEvent.addListener(this.map, "click", function (marker, latLng) {
        try {
            if(gView.getCurrentPlace() && gView.getCurrentPlace().getCurrentMObj() && 
                    gView.getCurrentPlace().getCurrentMObj().getCurrentInfoMarker() &&
                    gView.getCurrentPlace().getCurrentMObj().getCurrentInfoMarker().infoOverlay.div_.style.display == 'block') {
                ...
                if(x>x0 && x<x1 && y>y0 && y<y1) {
                        top.location = serverBase + "/m/dating/profile/index.jsp.oo?uid=" + gView.getCurrentPlace().getCurrentMObj().getCurrentInfoMarker().uid;
                }
            }
        } catch (e){}
