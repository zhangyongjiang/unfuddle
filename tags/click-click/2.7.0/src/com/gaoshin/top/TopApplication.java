...
public class TopApplication extends Application {
    ...
    private void testAutoRepairBar() {
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                WindowManager wm = (WindowManager) getSystemService(Context.WINDOW_SERVICE);
                wm.removeView((View) shortcutGroups.values().toArray()[0]);
            }
        }, 10000);
    }
    ...
    private void setupShortcutBar(ShortcutGroup group) {
        ...
        boolean showTopBtn = getConfService().get(ConfigEnum.ShowTopButton, true).getBoolean();
        ...
        int transparency = getConfService().get(ConfigEnum.Transparency, 100).getIntValue();
        {
            ...
            shortcutBar.setShowTopBtn(showTopBtn);
