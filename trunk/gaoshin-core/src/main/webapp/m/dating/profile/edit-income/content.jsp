...
<h3>My Income</h3>
    ...
    <fieldset data-role="controlgroup" >
    	<% 	for (DimensionIncome income : DimensionIncome.class.getEnumConstants()) {%>
         	<input type="radio" data-theme='c' name="radio-income" id="<%=income.name() %>" value="<%=income.name() %>" <%if(income.equals(profile.getIncome())){ %>checked="checked"<%}%> />
         	<label for="<%=income.name() %>"><%=income.getLabel() %></label>
    	<% } %>
    </fieldset>
...
function change() {
    var elem = $("input[name=radio-income]:checked");
	...
	$.ajax({
		url : serverBase + "/dating/profile/edit-income",
		type : "POST",
		data : json,
