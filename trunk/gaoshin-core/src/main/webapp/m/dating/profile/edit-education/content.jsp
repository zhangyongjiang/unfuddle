...
<%@page import="com.gaoshin.dating.DimensionEducation"%>
...
<div style="width:90%;overflow:hidden;">
    <fieldset data-role="controlgroup" >
    	<% 	for (DimensionEducation job : DimensionEducation.class.getEnumConstants()) {%>
         	<input type="radio" data-theme='c' name="radio-job" id="<%=job.name() %>" value="<%=job.name() %>" <%if(job.equals(profile.getEducation())){ %>checked="checked"<%}%> />
...
function change() {
	...
	$.ajax({
		url : serverBase + "/dating/profile/edit-education",
		type : "POST",
		data : json,
