...
<div style="width:88%;">
    <fieldset data-role="controlgroup" >
    	<% 	for (DimensionRace race : DimensionRace.class.getEnumConstants()) {%>
         	<input type="radio" data-theme='c' name="radio-race" id="<%=race.name() %>" value="<%=race.name() %>" <%if(race.equals(profile.getRace())){ %>checked="checked"<%}%> />
         	<label for="<%=race.name() %>"><%=race.getLabel() %></label>
    	<% } %>
    </fieldset>
</div>
...
function change() {
    var elem = $("input[name=radio-race]:checked");
	...
	$.ajax({
		url : serverBase + "/dating/profile/edit-race",
		type : "POST",
		data : json,
