...
<o:text-input-tr label="Mobile" id="mobile"/>
<o:text-input-tr label="Birth Year" id="birthYear"/>
<o:text-input-tr label="Gender" id="gender"/>
<o:submit-tr value="Sign Up"/>
</table>
</form>

<div style="clear:both;margin-top:32px;"><span style="font-size:0.8em;">Already got account?</span> <a href="#" onclick='window.location="<c:url value="/pub/login/index.jsp.oo"/>"'>Sign In</a></div>
...
function signup() {
	...
	if( mobile == null || mobile.trim().length == 0) {
		alert("All filelds are required. Please check mobile field.");
		return false;
	}
		...
		$.ajax({
			  ...
			  complete: function(transport) {
				     if(transport.status == 200) {
				         self.location = '<c:url value="/user/index.jsp.oo"/>';
				     } else {
