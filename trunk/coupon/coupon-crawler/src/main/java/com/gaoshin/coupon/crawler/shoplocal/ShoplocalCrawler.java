...
//        crawler.getStore("SAN FRANCISCO", "CA", "133249", fw);
        crawler.getStoresV2("NEW YORK", "NY", fw);
        fw.close();
    }

    public static com.gaoshin.coupon.crawler.WebCoupon getDetails(String url) throws Exception {
        Document doc = getDocument(url);
    ...
    public Category crawlCategories() throws Exception {
    ...
    private void listCategories(Category cat, String url, HashMap<String, Category> catmap, FileWriter fw) throws Exception {
        Document doc = getDocument(url);
        List<Element> elements = selectElements(doc, "A");
        for(Element ele : elements) {
            Link link = getLink(ele);
