...
<%
    ...
    if(zoom == null)
        zoom = "12";
	String url = "/ws/yipit/deals-by-geo?format=object&" + (param == null ? "" : param);
...
<div style="float:right;margin-right:40px;"><a href="/slocal/geo/index.jsp.oo?<%=request.getQueryString()%>">slocal</a></div>
    ...
    <table style="border:solid 1px;width:96%;">
		...
		<div><a href='${deal.url }'>${deal.title}</a></div>
			<div style="font-size:1.6em;">$${deal.price} <c:if test="${not empty deal.discount}"><span style='color:red;'>${deal.discount} off</span></c:if></div>
				...
				<c:if test="${not empty deal.rootSource}"><div style="margin-top:10px;font-size:0.8em;color:#bbb;">${deal.rootSource}</div></c:if>
            </td></tr>
        </c:forEach>
    </table>
