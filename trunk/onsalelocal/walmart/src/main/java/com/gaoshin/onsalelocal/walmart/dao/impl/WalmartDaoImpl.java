package com.gaoshin.onsalelocal.walmart.dao.impl;
...
@Repository("walmartDao")
public class WalmartDaoImpl extends GenericDaoImpl implements WalmartDao {
	private static final Logger log = Logger.getLogger(WalmartDaoImpl.class);

	@Override
	public void updateTasks() {
		long now = System.currentTimeMillis();
		long before = now - 24 * 3600000;
		String sql = "update walmart.Task set status='Ready', updated=? where type=? and updated<?";
		int updated = super.update(sql, now, WalmartStoreTaskHandler.WalmartStoreTask, before);
    ...
    public void seedTasks() {
		...
		String sql = "insert into Task (id, type, param, created, updated, status, tried, succeed)" +
					" select uuid(), '" + WalmartStoreTaskHandler.WalmartStoreTask + "'" +
					", id" +
					", " + now +
					", " + now + 
					", 'Created', 0, 0" + 
