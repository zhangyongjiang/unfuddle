...
public class GrouponTaskHandler implements TaskHandler {
	private static final Logger log = Logger.getLogger(GrouponTaskHandler.class);
	...
	public int processTask(List<Task> tasks) {
		...
		for(Task task : tasks) {
			String[] geo = task.getParam().split(",");
			float lat = Float.parseFloat(geo[0]);
			float lng = Float.parseFloat(geo[1]);
			try {
				DealsResponse response = GrouponApi.findDeals(lat, lng, 20);
